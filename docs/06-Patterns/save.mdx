...
Problems that come up with autosave (will probably turn this into a do/don’t section):
- Changes being automatically saved/applied unexpectedly
- Toast message being too far away from changes being made
- Lack of toast confirmation
...
#### Implementation
Inline save should be used in areas that are **medium impact**. In particular, this pattern is helpful for cases where entities are displayed in rows and users are editing one or more rows. 
...
#### Implementation
Apply + save should be used in areas where users might want to preview their changes before actually committing to them. This pattern should be used in areas that are medium- to medium-high impact. In Grafana, this pattern is most commonly used with dashboard editing. In order to be helpful to users, there should be a strong indication when something has been applied but is still unsaved, vs something that has already been saved. 
...
Currently, it seems that some pages trigger a refresh before redirect and others don’t. This warrants further investigation.
